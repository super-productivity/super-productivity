...
14. **Logical clock**: For "what day does this moment belong to?", go through `DateService` (`getLogicalNowMs`, `getLogicalTodayDate`, `getLogicalTomorrowMs`, `todayStr`, `isToday`). Pure reducers and selectors take `startOfNextDayDiffMs` as an explicit argument and call `isTodayWithOffset` — this is required for sync-replay determinism and is enforced by ESLint. Never read `startOfNextDayDiff(Ms)?` directly outside `DateService` itself.
...
| `Date.now() - startOfNextDayDiff` inline | `dateService.getLogicalNowMs()` — or `isTodayWithOffset(ts, todayStr, offsetMs)` in pure reducers/selectors |
| `const { startOfNextDayDiff } = dateService` | `dateService.getStartOfNextDayDiffMs()` — destructuring is ESLint-banned |
