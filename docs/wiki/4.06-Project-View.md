...
When you finish a project, you can **complete** it to mark it as done and celebrate the result. Right-click the project in the sidebar (or click the `⋮` button) and choose **Complete project**.

Completing a project:

- Shows a **celebration** with a short summary — tasks done, time tracked, days worked, and how many days the project ran from first worked day to completion (time-based stats are hidden when you don't track time).
- **Archives** the project at the same time, so it leaves your active workflow exactly like an archived project (see below). The difference is intent: completing is a celebrated finish, archiving is a quiet shelving.

If the project still has **unfinished tasks**, you're asked what to do with them first: **Move to Inbox** (the default — they stay actionable), **Mark as done**, or **Cancel**.

Completed projects appear on the **Archived projects** page with a trophy badge and their completion date. To pick a project back up, use the **Reopen** action there (or **Undo** on the snack shown right after completing) — it clears the completed state and returns the project to the sidebar.
