...
Titles are parsed with Short Syntax enabled, same as a manually typed title (see [[3.04-Short-Syntax]]) — `title=Buy milk +Work #urgent` creates/assigns tags and a project from the title text itself, in addition to any explicit `projectId`.
