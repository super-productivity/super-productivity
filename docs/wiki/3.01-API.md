...
For development scripts, `SP_FORCE_LOCAL_REST_API=1 npm start` starts the local REST API without changing the persisted user setting. This override only works in `NODE_ENV=DEV`. On a clean profile, set `SP_FORCE_LOCAL_REST_API_TOKEN=<token>` to choose the Bearer token explicitly; otherwise the app generates a temporary development token and prints it to the Electron log.
...
A missing, malformed, or wrong token returns `401` with the `UNAUTHORIZED` error code, as
does any request made before the app has handed the token to the server. `GET /health` is
the only exception and stays unauthenticated so tooling can probe liveness.
