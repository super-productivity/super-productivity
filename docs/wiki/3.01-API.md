...
- `registerTaskContextMenuEntry(config)` — Adds an action under **Plugin actions** in task context menus. Requires the `taskContextMenu` manifest permission. The callback receives `{ taskId }`; `showFor` can limit the action to `TASK` or `SUBTASK`.
