...
# Create a recurring task (the created task is the first instance of the series).
# `rrule` is an RFC 5545 RRULE body without the "RRULE:" prefix.
curl -X POST http://127.0.0.1:3876/tasks \
  -H "Content-Type: application/json" \
  -d '{"title": "Water plants", "rrule": "FREQ=DAILY;INTERVAL=3", "repeatFromCompletionDate": true}'
...
| `rrule`              | Makes the task recurring; the created task is the series' first instance. An RFC 5545 RRULE body without the `RRULE:` prefix (e.g. `FREQ=WEEKLY;BYDAY=MO`). Invalid rules return 400 `INVALID_INPUT`. Cannot be combined with `parentId` (subtasks can't recur) — returns 400 `UNSUPPORTED_FIELD`. |
| `startDate`          | Recurrence anchor for `rrule`, `YYYY-MM-DD` (defaults to today). Other formats return 400 `INVALID_INPUT`.                                                                                                                                                                                         |
| `repeatFromCompletionDate` | With `rrule`: when `true`, the interval is counted from the day each instance is completed instead of from fixed calendar dates.                                                                                                                                                             |
