...
**GET /tasks/:id** returns the task. When the task is linked to an issue and its provider can build a link to it, the response also includes `issueUrl` (string). The field is omitted when there is no issue, no link, or the lookup fails or takes longer than about 3 seconds; it never turns the request into an error. `GET /tasks` does not include `issueUrl`.
