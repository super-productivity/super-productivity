...
- Projects returned by `getAllProjects()` and `getAppState().projects` include an optional read-only `folderPath` display value when the project tree exposes one, primarily for labeling/disambiguating duplicate project names.
