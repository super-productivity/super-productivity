...
Setting `projectId` moves a top-level task and all of its subtasks to the destination project. The destination must exist; otherwise the request returns 404 `PROJECT_NOT_FOUND`. A subtask's `projectId` is inherited from its parent and cannot be changed directly (returns 400 `UNSUPPORTED_FIELD`). Project moves can be combined atomically with other writable fields in the same `PATCH` request.
...
| `PROJECT_NOT_FOUND` | 404         | Destination project does not exist                                              |
