...
- `registerTaskContextMenuEntry(config)` — Adds an entry to the task right-click menu; `onClick` receives the task id. Optional: absent on older app versions.
...
- `openExternalUrl(url)` — Open a URL with the OS default handler (desktop) or in a new tab (web). Requires the `openExternalUrl` permission; the URL must pass the app's external-link scheme allowlist, and `file:` URLs are always refused. `plugin.js` only; optional, absent on older app versions.
...
- **openExternalUrl:** Required for `openExternalUrl()`. Listed on the plugin card
  like every declared permission.
...
**Short Syntax:** titles sent to `POST /tasks` and title-only `PATCH /tasks/:id` requests are parsed like a typed task (see [[3.04-Short-Syntax]]): `#tag`, `+Project`, `@date`, time estimates such as `30m` and URLs are taken out of the title and applied. Send `"isIgnoreShortSyntax": true` in the body to store the title literally instead — useful when the title comes from another app. A non-boolean value returns 400 `INVALID_INPUT`.
