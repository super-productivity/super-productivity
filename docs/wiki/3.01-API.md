...
- `POST /api/register/magic-link` — Register a new email-only account with a magic-link flow.
...
- `POST /api/sync/snapshot` — Upload full state (backup/repair/migration). Body limit 30 MB (compressed). There is no production `GET /api/sync/snapshot`; historical snapshots are retrieved via `GET /api/sync/restore/{serverSeq}`.
...
- `GET /api/sync/ws?token={token}&clientId={clientId}` — WebSocket notifications for new sync operations.
...
- **Snapshot:** Upload snapshot body: `state`, `clientId`, `reason`, `vectorClock`, optional `schemaVersion`, `isPayloadEncrypted`, `syncImportReason`, `opId`, `isCleanSlate`, `snapshotOpType`, `requestId`. Restore response: `state`, `serverSeq`, `generatedAt`.
