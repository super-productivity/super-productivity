...
The Local REST API allows external scripts and tools to interact with a running Super Productivity desktop app. It runs on `http://127.0.0.1:3876` by default and is disabled by default. Enable it in **Settings → Misc → Enable local REST API**.

Optionally, enable **Allow external network access** in the same settings section to bind the API to all network interfaces (0.0.0.0), making it reachable from other devices on the same network (e.g. WSL, LAN). See the Authentication section below for the security implications.

For development scripts, `SP_FORCE_LOCAL_REST_API=1 npm start` starts the local REST API without changing the persisted user setting. This override only works in `NODE_ENV=DEV`. The env var `SP_LOCAL_REST_API_HOST` (must be a valid IPv4 address; IPv6 not supported) overrides the bound IP address when the API is enabled.
...
No authentication required. By default the API only accepts connections from localhost (`127.0.0.1`). With **Allow external network access** enabled, the server binds to all interfaces (`0.0.0.0`) and accepts connections whose `Host` header exactly matches one of the machine's current IPv4 addresses (e.g. `192.168.1.x:3876`) — DNS names and `hostname.local` are not accepted and will return 403. DNS-rebinding protection is still active. There is no token or password — anyone who can reach the port can read and modify your tasks. Do not enable external access on untrusted networks.

### Base URL

```text
http://127.0.0.1:3876          (default — localhost only)
http://<your-machine-ip>:3876  (with external access enabled)
```
...
- **No auth:** There is no authentication. By default the API is localhost-only. With external access enabled, DNS-rebinding protection (Host-header allowlist) and CSRF protection (Origin-header rejection) remain active, but any device that can reach the port has full read/write access.
- **Port:** Fixed at `3876`; not configurable in v1.
- **Timeout:** 15 seconds for renderer responses.
- **External access:** Enable in Settings → Misc → Allow external network access to REST API. The server binds to `0.0.0.0` and is reachable from other devices on the same network, including WSL.
- **Env vars (advanced):** `SP_LOCAL_REST_API_HOST` overrides the bound IP address when the API is enabled (must be a valid IPv4 address; IPv6 and hostnames are rejected). `SP_FORCE_LOCAL_REST_API=1` enables the API in `NODE_ENV=DEV` without changing the persisted setting.
...
- **Local REST API:** Desktop-only, bound to `127.0.0.1` by default (binds to all interfaces when external access is enabled), no authentication. For automation and scripting. See [[3.05-Web-App-vs-Desktop]] for platform differences.
