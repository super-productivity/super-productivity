...
| ------------------------- | ------------------------------- | -------------------------------------------------------- |
| iOS / Android (Capacitor) | `com.super-productivity.app://` | `com.super-productivity.app://add-task?title=Buy%20milk` |
...
**Add a task** — mobile: action `add-task`; desktop: action `create-task` (also accepts the title as a path segment, `create-task/<title>`, for backward compatibility with a pre-existing but previously-unwired desktop action).

| Query parameter | Required | Description                                                                                                    |
| --------------- | -------- | -------------------------------------------------------------------------------------------------------------- |
| `title`         | Yes      | The new task's title.                                                                                          |
| `notes`         | No       | Task notes.                                                                                                    |
| `projectId`     | No       | Target project ID. Silently ignored (task is added without a project) if it doesn't match an existing project. |

**Complete a task** — action `complete-task` on both platforms.

| Query parameter | Required | Description                                                                                        |
| --------------- | -------- | -------------------------------------------------------------------------------------------------- |
| `title`         | Yes      | Matched case-insensitively, substring, against all non-done tasks. The first match is marked done. |
...
- Both platforms show a snack notification on success (task added/completed) or failure (no matching task found) — there is no other feedback channel for a URL triggered from outside the app.
