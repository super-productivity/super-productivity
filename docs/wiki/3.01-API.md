...
Changing `projectId` on a top-level task moves it (and its subtasks) to the target project, updating both projects' task lists. The destination must be an existing, non-archived project (otherwise 404 `PROJECT_NOT_FOUND`). `projectId` cannot be changed on a subtask (400 `UNSUPPORTED_FIELD`) — move its parent instead. Sending the task's current `projectId` is a no-op, so `GET`→`PATCH` round-trips are safe.
