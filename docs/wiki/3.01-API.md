...
For development scripts, `SP_FORCE_LOCAL_REST_API=1 npm start` starts the local REST API without changing the persisted user setting. This override only works in `NODE_ENV=DEV`. The env var `SP_LOCAL_REST_API_HOST` (must be a valid IPv4 or IPv6 address) overrides the bound host regardless of UI settings.
...
No authentication required. By default the API only accepts connections from localhost (`127.0.0.1`). With **Allow external network access** enabled, the server binds to all interfaces and accepts connections from any device whose Host header resolves to one of the machine's local IPv4 addresses (DNS-rebinding protection is still active). There is no token or password — anyone who can reach the port can read and modify your tasks. Do not enable external access on untrusted networks.
...
- **Env vars (advanced):** `SP_LOCAL_REST_API_HOST` overrides the bound IP address (must be a valid IPv4 or IPv6 IP, not a hostname). `SP_FORCE_LOCAL_REST_API=1` enables the API in `NODE_ENV=DEV` without changing the persisted setting.
