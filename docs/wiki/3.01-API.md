...
**Add a task** — action `create-task` on both platforms (desktop also accepts the title as a path segment, `create-task/<title>`, for backward compatibility with a pre-existing but previously-unwired desktop action). Not `add-task`: desktop's `add-task` protocol action already exists and does something unrelated (opens the quick-add-task input bar).

| Query parameter | Required | Description                                                                                                |
| --------------- | -------- | ---------------------------------------------------------------------------------------------------------- |
| `title`         | Yes      | The new task's title. Whitespace-only is rejected (no task is added).                                      |
| `notes`         | No       | Task notes.                                                                                                |
| `projectId`     | No       | Target project ID. The action fails with an error (no task added) if it doesn't match an existing project. |

The title is added **verbatim**: Short Syntax (see [[3.04-Short-Syntax]]) is _not_ parsed, so tokens like `+Project`, `#tag`, or `@date` in the title are stored literally rather than creating/assigning a project, tags, or a due date. A URL is untrusted external content, so — as with the email-drop import — it can't silently create tags/projects or override an explicit `projectId`. Use the `projectId` parameter to set the project.
...
| Query parameter | Required | Description                                                                    |
| --------------- | -------- | ------------------------------------------------------------------------------ |
| `title`         | Yes      | Matched against non-subtask, non-done tasks in active (non-archived) projects. |
...
- **URL Scheme Actions:** Mobile (Capacitor) and desktop (Electron) only, not the web build. No authentication — any process able to open a URL on the device can trigger it, same trust model as the existing OAuth-callback/global-shortcut actions on the same schemes.
