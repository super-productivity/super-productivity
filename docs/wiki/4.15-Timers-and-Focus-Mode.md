...
- **Show a browser notification** — In the web app this is an opt-in Focus Mode setting, and the browser must allow notifications for the site.
...
Focus Mode has **multiple dedicated settings** (e.g. sync with task tracking, start in background, skip preparation screen, play tick sound, optional browser notifications, pause task tracking during break, manual break start). Those options show that focus is meant to be **configurable** to your workflow. The feature is also **woven into the app**: session lifecycle (start, tick, complete), break handling, task-tracking sync, and metrics all depend on focus state. So focus is **central to the workflow**, not a side panel you can ignore.
