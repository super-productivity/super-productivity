...
- **Grammar:** `@every <unit>` or a bare frequency word: `@daily`, `@weekly`, `@monthly`, `@yearly`, `@annually`. Units for `every`: `day(s)`, `week(s)`, `weekday(s)`/`workday(s)`, `month(s)`, `year(s)`, a weekday name (`friday`, `fri`, `fridays`, ...), or a day of the month (`15th`). An interval can precede the unit: `@every 2 weeks`. An optional time can follow: `@every friday 3pm`.
- **Mapping:** `@every friday` → weekly on Friday (due date set to the next Friday); `@every weekday` → Monday through Friday; `@every 15th` → monthly on the 15th (due date set to the next 15th); `@every 2 weeks` → weekly with interval 2; `@daily 6am` → daily with a start time.
- **Valid:** `"Water plants @every friday"`, `"Journal @daily 6am"`, `"Pay rent @every 15th"`, `"Review @every 2 weeks"`.
...
4. **Return value:** The `shortSyntax()` function returns an object with task changes (and `newTagTitles`, `remindAt`, `projectId`, `repeatCfg`) or `undefined` if nothing was parsed or syntax is disabled.
