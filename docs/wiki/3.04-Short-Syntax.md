...
- **Invalid / inert:** `"either/or"`, `"w/ milk"` (no word boundary before `/` or no match) — nothing is stripped from the title unless a section actually matches. `"Task / Design"` (space after `/`) is not parsed. `"+/..."` (nothing before the slash) is plain text, so notation like `+/-` stays untouched. Time forms like `1h/2h` are parsed as time before section parsing runs. A slash token that matches no section is left in the title and does not block a later one (`"review PR /9014 /Design"` still files into `Design`).
