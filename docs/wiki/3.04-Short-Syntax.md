...
- **Mapping:** `@every friday` → weekly on Friday (due date set to the next Friday); `@every weekday` → Monday through Friday; `@every 15th` → monthly on the 15th (due date set to the next 15th); `@daily 6am` → daily with a start time.
...
### Natural Language Dates (no trigger, opt-in)

- **Symbol:** none — plain text.
- **Purpose:** Detect due dates and recurrence without the `@` trigger: `"Call mom tomorrow"`, `"Standup at 9am"`, `"Shopping every friday"`.
- **Enabling:** Off by default; toggle "Detect dates in plain text without @" in [[3.02-Settings-and-Preferences#global-settings.Short-Syntax]] (requires the planned-at syntax to be enabled).
- **Rules:** Explicit `@` syntax always wins over plain-text detection. Recurrence phrases win over the plain date they contain (`every friday` is not consumed as just `friday`). Bare numbers without a letter or date separator are never treated as dates (`"Buy 5 apples"`). Detected text is highlighted inline and stripped from the title on creation.
- **Dismissing a false positive:** Clearing the date (or repeat) via its button keeps the word in the title and remembers the dismissal for the rest of the composing session, so it does not re-trigger on the next keystroke. Dismissing a recurrence phrase also blocks the plain date contained in it.
- **Scope:** Add Task Bar (task creation) only — title edits of existing tasks are never rewritten based on plain words.
...
- **Inline highlighting:** Detected short syntax (tags, project, estimate, due/deadline dates, recurrence, extracted URLs) is highlighted directly in the input while typing, showing exactly which characters will be stripped from the title before the task is created.
