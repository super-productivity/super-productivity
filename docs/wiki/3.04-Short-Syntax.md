...
- **Tag mode:** Tags parsed from a task title are added to the tags the task already has. The Add Task Bar is the exception: its tags mirror the text currently in the input, so deleting a `#tag` there removes it again before the task is created. There is no setting for this.
