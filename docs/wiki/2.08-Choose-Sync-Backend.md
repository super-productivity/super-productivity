...
- **Proton Drive (desktop only)** — File-based sync to your Proton Drive account. Proton Drive has no official third-party API, so the desktop app bridges to it through [rclone](https://rclone.org/protondrive/): the app runs a local `rclone serve webdav` process against your Proton Drive remote and syncs to it like any WebDAV target. Requires a one-time setup (see below). Only appears in the desktop app — not on web or mobile.
...
- **You use Proton Drive and are on the desktop app** → **Proton Drive**. Keeps your data in Proton's end-to-end-encrypted storage, at the cost of a one-time rclone setup.

## Proton Drive Setup (desktop only)

Because Proton Drive has no official sync API, the app relies on [rclone](https://rclone.org), which provides a community-maintained, reverse-engineered Proton Drive backend that handles Proton's login and end-to-end encryption.

1. [Install rclone](https://rclone.org/downloads/) and make sure it is on your system `PATH` (or note its full path).
2. In a terminal run `rclone config` and create a new remote of type **protondrive**. This walks you through your Proton login, including 2FA and (if set) your separate mailbox password. Give the remote a name — `protondrive` is the default the app expects.
3. In Super Productivity: **Settings → Sync → Sync provider → Proton Drive**. Enter the rclone remote name and a sync folder, optionally set an explicit rclone path, then click **Test Connection**.

The app never stores your Proton password — those credentials live only in rclone's own (obscured) config. The local bridge listens on `127.0.0.1` with random per-session credentials and is stopped when the app quits.
