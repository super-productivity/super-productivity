...
The "Decryption Failed" dialog shown after a failed sync never offers to overwrite the server. It keeps the part of your data that it could decrypt and leaves the server copy untouched. The encryption password prompt refuses to overwrite the server when the device has never completed a sync and holds no data of its own; if you see that refusal, the server copy is untouched.
