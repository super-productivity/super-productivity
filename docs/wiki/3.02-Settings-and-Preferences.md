...
Task completion browser notifications are off by default and can be enabled separately from the done sound.
...
Focus session and break completion browser notifications are off by default. Enable them separately if you want system-level alerts when a session or break ends.
