...
- **Enable project short syntax (`+<project>`)** — Parse `+project` in task titles.
- **Enable tag short syntax (`#<tag>`)** — Parse `#tag` in task titles.
- **Enable planned at short syntax (`@<planned time>`)** — Parse `@date`/`@time` (and recurrence phrases like `@every friday`) in task titles.
- **Detect dates in plain text without @** — Off by default. When enabled, the Add Task Bar also detects due dates and recurrence in plain text (e.g. "Call mom tomorrow", "Shopping every friday") without the `@` trigger. Detections are highlighted inline and can be dismissed via the date/repeat buttons; a dismissed detection keeps the word in the title and stays dismissed while composing. Only shown when the planned-at syntax is enabled; applies to task creation only, never to title edits of existing tasks. See [[3.04-Short-Syntax]].
- **URL behavior** — What to do with URLs in task titles: keep in title, extract to attachments, or keep and attach.
