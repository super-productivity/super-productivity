...
- **Deadlines** are updated when the target panel requires a deadline or no deadline
...
When a scheduling or deadline filter has a restricted timeframe, dropping a task into that panel adjusts the task to the nearest date that still belongs to the panel. For example, dropping a task into a "Scheduled: Next 7 days" panel plans it inside that range instead of letting it disappear after the drop.

## Date And Deadline Filters

Panel scheduling filters can show all tasks, only scheduled tasks, or only unscheduled tasks. When a panel shows scheduled tasks, you can optionally limit it to a timeframe such as all scheduled dates, today, tomorrow, the next 7 days, the next X days, at least X days in the future, next week, next month, a custom date, or a custom date range.
...
Custom ranges can be bounded on both sides or only one side. A start date without an end date means "from this date onward"; an end date without a start date means "up to this date".
