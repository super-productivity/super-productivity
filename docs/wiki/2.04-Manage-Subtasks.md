...
## Subtask Dates and Deadlines

If a parent task is scheduled with a due date or deadline:

- **Newly created subtasks** will automatically inherit those same dates/deadlines if the corresponding options under **Settings -> Tasks -> Automatically pass parent dates to newly created subtasks** are enabled.
- **When modifying a parent task's due date or deadline**, the app will ask you via a confirmation dialog if you want to propagate the same change or removal to all existing subtasks.
