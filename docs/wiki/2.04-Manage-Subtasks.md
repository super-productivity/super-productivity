...
You can nest subtasks up to four total task levels. When a task is already at the deepest supported level, add-subtask actions are hidden or blocked for that task.
...
Drag the subtask by its **drag handle** and drop it onto the target parent task, or into that parent’s subtask list. The subtask becomes a child of the new parent and keeps the same project (subtasks inherit the parent’s project). Moves are blocked when the moved subtask hierarchy would exceed the four-level depth limit.
