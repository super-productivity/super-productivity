...
The `add-task`/`complete-task` URL scheme actions (see [[3.01-API]]) rely on the app having a registered custom URL scheme, which only Electron and the Capacitor mobile builds have. Not available in the web (browser) build.
