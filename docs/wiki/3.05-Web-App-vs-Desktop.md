...
For self-hosted Jira instances where the server is configured with appropriate CORS headers, the extension requirement can be bypassed: enable **Allow direct fetch as fallback** in the Jira advanced settings. When active, the app sends API requests directly from the browser instead of routing them through the extension. This option has no effect in the desktop app or on Android.
