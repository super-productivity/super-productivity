...
The web app uses the Web Notifications API for desktop-style alerts such as focus-session completion, break completion, task completion, reminders, and break reminders. Focus-session, break, and task-completion browser notifications are opt-in settings; enabling them prompts the browser permission flow when needed. These notifications require browser permission and a secure context. If permission is denied or unsupported, the app can still show in-app feedback such as banners, dialogs, and sounds, but the browser will not display a system notification.
...
- Native OS notification integration (it uses the Web Notifications API instead).
