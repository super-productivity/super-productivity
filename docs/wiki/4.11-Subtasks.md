...
Subtasks can themselves have subtasks (nesting): a task can be both a parent and a child. The same two-way link applies at each level. Super Productivity supports up to **four total task levels**: a top-level task plus three levels of nested subtasks. The app blocks creating or moving subtasks when the result would exceed that depth.
...
When you use a markdown-based workflow (for example the sync-md plugin) that converts indentation into parent-child structure, depth is determined by indentation level. Markdown task structure follows the same four-level task cap; items beyond that depth may be kept as notes instead of becoming additional subtasks.

### Inheritance and Consistency When Moving or Duplicating

- **Project inheritance** — Every task, including every subtask, belongs to exactly one project. When you **move a subtask to a different parent**, the subtask and its nested subtasks inherit the **new parent’s project**. That keeps the whole moved hierarchy in the correct project without you having to change project manually. If you move it back to another parent in a different project, it will inherit that parent’s project in turn.
