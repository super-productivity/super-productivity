...
- **With "Wait for completion"**: The app checks if any uncompleted instance exists (including archived tasks). If one does, it will not create the next instance. Once you mark the current instance as done, the next instance appears immediately.

### Key Behaviors

- **Immediate creation after completion**: When you complete the gating task, the next instance is created right away—you don't have to wait until the next day or restart the app.
...
- **Latest instance only**: Only completing the most recent instance triggers the next creation. Completing an old archived instance won't advance the schedule.
