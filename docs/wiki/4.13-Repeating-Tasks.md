...
## Create a Task for Each Missed Occurrence

By default, if you open the app after several scheduled occurrences were missed (for example, you were away for a week), only a **single** instance is created for the most recent occurrence. The occurrences in between are skipped. This is the right default for most people—you don't want to come back to a wall of overdue chores.

When you enable **"Create a task for each missed occurrence"** (in the advanced section), the app instead creates a **separate overdue task for every missed occurrence**, so nothing slips through. This is useful for tasks where each occurrence is a distinct unit of work you still need to account for—billing periods, log entries, reports.

### How Backfill Works

- **Normal behavior**: Only the newest due occurrence is created when you catch up; the schedule pointer jumps forward past the missed ones.
- **With "Create a task for each missed occurrence"**: Every occurrence between the last creation and today is created as its own task, oldest first.

### Backfill Behaviors

- **Capped at 30**: To avoid flooding your list (and your sync history) after a long absence, at most the **30 most recent** missed occurrences are created. Older occurrences are skipped, but the schedule still advances past them so they aren't re-evaluated.
- **No duplicates**: Occurrences that already exist or that you previously deleted are skipped.
- **Catch-up only**: Backfill only happens when catching up to today; tasks pre-created for tomorrow are unaffected.
- **Mutually exclusive with "Skip overdue instances"**: The two are opposites (create every missed vs. skip every missed), so enabling one hides the other.
