...
3. Start the stack:
   ...
   > Start only the `app` and `webdav` services. The same file also defines a
   > `supersync` service used by the end-to-end test suite; it runs in test mode
   > with a signing secret that is published in this repository and must never
   > be started outside CI. To self-host SuperSync, follow
   > `packages/super-sync-server/README.md` instead.
