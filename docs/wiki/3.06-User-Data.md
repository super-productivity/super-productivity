...
On desktop, Settings → Automatic Backups shows the last successful backup time as of opening Settings. If a drive disconnects or the folder becomes unwritable, failed writes leave this timestamp unchanged. A custom backup folder that has gone missing is never re-created, so backups cannot silently end up on the wrong disk; they resume once the folder is back. The exception is a folder that is itself a mount point: it still looks present when unmounted, so backups then go to the empty directory underneath. Prefer a subfolder of the mount point over the mount point itself. Existing `.json` backup files in the default folder remain available for restore, while automatic cleanup only removes current timestamp-shaped files.
