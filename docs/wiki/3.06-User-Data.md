...
Stopwatch tracking records individual work sessions on each task. Pausing and
resuming creates separate recordings. Each recording stores its work day,
start timestamp, duration, and the timezone offset at recording time. Periodic
saves extend the current recording. Duration-only entries have no start time.
Existing tasks need no migration: their daily totals remain available without
invented historical timestamps.

In the daily summary, expand **Refine time tracking** to review recordings, edit
a day's total, or add a session to an existing or new task. The difference between
work start/end minus breaks and task totals shows time still to allocate. It is
not automatically counted as work. A negative difference means task totals
exceed the entered work time.

Changing a daily total, including through short syntax, leaves recordings
intact. The detail view shows the difference as duration without a recording or
a correction. Explicitly editing or deleting a session changes that day's total
by the same amount, with a minimum of zero. Existing rounding controls affect
totals without rounding recorded intervals.

Sessions travel with tasks through archiving, restoration, and full JSON
backups. Older clients can still apply tracked durations but do not record or
display sessions; conflicts or snapshots from older clients can discard optional
session detail. Daily totals take priority when detail is incomplete. Native
background time recovered without interval metadata and legacy manual entries
remain visible as duration without a recording.
