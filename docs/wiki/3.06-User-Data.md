...
### Android Native Op-log Database

The Capacitor Android app keeps the authoritative `SUP_OPS` operation log,
snapshots, and archives in an app-private SQLite database. On the first launch
after upgrading, the app copies the legacy IndexedDB database into SQLite in one
verified transaction and retains the IndexedDB copy temporarily for pre-migration
recovery only. Web, Electron, iOS, and the legacy Android online-only WebView
continue to use IndexedDB.

The native op-log database is excluded from Android cloud backup and device
transfer because restoring a stale operation history could corrupt sync
reconciliation. Android's separate `SupKeyValStore` automatic-backup slots remain
