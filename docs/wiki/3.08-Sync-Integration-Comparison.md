...
If sync stops with `SYNC_MULTI_ENTITY_UNSUPPORTED`, choose **Sync: Conflicting Data** in the error message or start a manual sync to open the recovery dialog. Background retries do not open the dialog automatically.
...
These choices replace the entire dataset, not just the conflicting tasks. Export backups from devices with changes you want to preserve before choosing a version. Remote change counts and timestamps are shown as unknown when unavailable.
