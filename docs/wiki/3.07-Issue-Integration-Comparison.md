...
- **Recurring tasks:** A recurring Plainspace task stays **one** local task across all its occurrences — Plainspace keeps a single item and moves its schedule forward. Once the server has advanced to the next occurrence, the local task is reopened and rescheduled on the next poll. This also applies if you archived it in the meantime: it is restored from the archive as the next occurrence, keeping its notes and tracked time. Archiving therefore does not stop a recurring task from coming back — end the repetition in Plainspace instead.
- **Worklog / Comments / Subtasks / Attachments:** Not supported.
