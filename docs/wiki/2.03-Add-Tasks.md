...
| **Deadline** | `Ctrl+8` | Set a deadline — the hard date by which the task must be done |
| **Tags**     | `Ctrl+5` | Assign tags                                                   |
| **Estimate** | `Ctrl+6` | Set a time estimate                                           |
| **Repeat**   | `Ctrl+7` | Configure a repeat schedule                                   |

Once a value is set, a **×** clear button appears next to the corresponding action button. On the Schedule/Planner view, the add bar may show different options (e.g. "Plan for today").
