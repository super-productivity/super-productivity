...
| `superproductivity://create-task/<title>` | —                       | Create a task with the URL-encoded `<title>` (also accepts `?title=`, `?notes=`, `?projectId=`). The title is stored literally — Short Syntax is not parsed. See the [API reference](3.01-API.md) for the full contract. |
