...
### Quick Add HUD (`globalTaskQuickAdd`)

The **Quick Add HUD** is a system-wide floating overlay for rapid task capture — similar to macOS Spotlight. It can be triggered with the `globalTaskQuickAdd` global shortcut from anywhere on your system, even when Super Productivity is not visible.

**Behaviour:**

- Opens a full-screen transparent overlay with the task add bar centred on screen.
- You can type a task title and press `Enter` to submit. The HUD stays open for rapid consecutive entries.
- Press `Escape` or click outside the bar to dismiss.
- When dismissed, focus returns to the app or window you were in before.
- The shortcut is configurable in **Settings → Keyboard Shortcuts**.

**Supported metadata while adding (same as the in-app add bar):**

- `Ctrl+1` – `Ctrl+8`: Toggle project, schedule, tags, estimate, repeat, deadline, etc.
