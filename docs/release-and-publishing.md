...
kind. `.github/workflows/build.yml` resolves it through the same
`node tools/release-notes.js base-tag` command, so the notes and the release
body's Full Changelog link always span the same range.
...
short notes - if you see that warning, check the notes against the release users
actually received.
