...
2. `SupersededOperationResolverService.resolveSupersededLocalOps()`:
   ...
   - On a long history that forced download spans many pages, and on mobile it is cut off whenever the app is backgrounded. `OperationLogDownloadService` therefore keeps an in-memory checkpoint (next `sinceSeq` + the merged clock of the pages done) and the next attempt resumes there instead of at seq 0. It only advances over pages that delivered **no** new op, so an unapplied op is always re-downloaded; it is keyed on client id + persisted cursor and dropped on a gap reset, on completion, and on any other seq-0 download
