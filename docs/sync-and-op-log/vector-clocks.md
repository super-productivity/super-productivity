...
- `detectConflictForEntity` (single) — **two `LIMIT 1` lookups**, taking the higher `server_seq`: a scalar `where: { entityId }` (the hot path, an ordered `entity_id` btree walk that reads one tuple) and an array `where: { entityIds: { has: entityId } }`. The array branch is not ordered — a `GIN(entity_ids)` index has no `server_seq`, so Postgres fetches the `@>` matches and sorts them; it stays cheap only because the column is populated for multi-entity ops alone, keeping the match set small. A single `OR` query would span both indexes and force a `BitmapOr` + sort over every stored version of the entity.
