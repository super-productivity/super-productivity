...
describe('TaskRepeatCfgReducer', () => {
  ...
  describe('updateTaskRepeatCfgInstance (RECURRENCE-ID)', () => {
    it('sets an override keyed by the occurrence day', () => {
      const cfg = createTaskRepeatCfg('cfg1');
      const action = TaskRepeatCfgActions.updateTaskRepeatCfgInstance({
        repeatCfgId: 'cfg1',
        dateStr: '2024-01-15',
        override: { movedToDay: '2024-01-18', title: 'Moved' },
      });
      const result = taskRepeatCfgReducer(createStateWithCfgs([cfg]), action);
      expect(result.entities['cfg1']!.instanceOverrides).toEqual({
        '2024-01-15': { movedToDay: '2024-01-18', title: 'Moved' },
      });
    });

    it('merges into an existing override for the same day', () => {
      const cfg = createTaskRepeatCfg('cfg1', {
        instanceOverrides: { '2024-01-15': { title: 'A' } },
      });
      const action = TaskRepeatCfgActions.updateTaskRepeatCfgInstance({
        repeatCfgId: 'cfg1',
        dateStr: '2024-01-15',
      ...
      const result = taskRepeatCfgReducer(createStateWithCfgs([cfg]), action);
      expect(result.entities['cfg1']!.instanceOverrides!['2024-01-15']).toEqual({
        title: 'A',
        notes: 'B',
      });
    });
    ...
    it('clears the override when passed null', () => {
      const cfg = createTaskRepeatCfg('cfg1', {
        instanceOverrides: { '2024-01-15': { title: 'A' }, '2024-02-01': { title: 'B' } },
      });
      const action = TaskRepeatCfgActions.updateTaskRepeatCfgInstance({
        repeatCfgId: 'cfg1',
        dateStr: '2024-01-15',
    ...
    it('returns unchanged state for a non-existent config', () => {
      const cfg = createTaskRepeatCfg('cfg1');
      const existingState = createStateWithCfgs([cfg]);
      const action = TaskRepeatCfgActions.updateTaskRepeatCfgInstance({
        repeatCfgId: 'nope',
      ...
      expect(taskRepeatCfgReducer(existingState, action)).toBe(existingState);
    });
  });
});
