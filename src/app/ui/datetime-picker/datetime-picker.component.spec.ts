...
describe('DateTimePickerComponent', () => {
  ...
  it('should set activeDate to the last day of the month when clicking previous month button', fakeAsync(() => {
    const calendar = component.calendar()!;
    calendar.currentView = 'month';
    calendar.activeDate = new Date(2026, 4, 15); // May 15, 2026
    fixture.detectChanges();
    ...
    expect(calendar.activeDate).toEqual(new Date(2026, 3, 30)); // April 30, 2026
    expect(component.isKeyboardNavigating).toBeTrue();
    expect(component.isInitialFocus).toBeFalse();
  }));

  it('should set activeDate to the first day of the month when clicking next month button', fakeAsync(() => {
    const calendar = component.calendar()!;
    calendar.currentView = 'month';
    calendar.activeDate = new Date(2026, 4, 15); // May 15, 2026
    fixture.detectChanges();
    ...
    expect(calendar.activeDate).toEqual(new Date(2026, 5, 1)); // June 1, 2026
    expect(component.isKeyboardNavigating).toBeTrue();
    expect(component.isInitialFocus).toBeFalse();
  }));

  it('should set activeDate to the minDate when clicking previous month button and the new month is the minDate month', fakeAsync(() => {
    const calendar = component.calendar()!;
    calendar.currentView = 'month';
    calendar.activeDate = new Date(2026, 5, 15); // June 15, 2026
    fixture.componentRef.setInput('minDate', new Date(2026, 4, 10)); // May 10, 2026
    fixture.detectChanges();
    ...
    expect(calendar.activeDate).toEqual(new Date(2026, 4, 10)); // May 10, 2026 (minDate)
    expect(component.isKeyboardNavigating).toBeTrue();
    expect(component.isInitialFocus).toBeFalse();
  }));

  it('should change activeDate to minDate and trigger focus when clicking previous button and the month does not change', fakeAsync(() => {
    const calendar = component.calendar()!;
    calendar.currentView = 'month';
    calendar.activeDate = new Date(2026, 4, 15);
    fixture.componentRef.setInput('minDate', new Date(2026, 4, 10)); // May 10, 2026
    fixture.detectChanges();
    ...
    // Directly spy on focus of the active cell
    const activeCell = document.createElement('div');
    activeCell.classList.add('mat-calendar-body-active');
    spyOn(activeCell, 'focus');
    spyOn(fixture.nativeElement, 'querySelector').and.callFake((selector: string) => {
      if (selector === '.mat-calendar-body-active') {
        return activeCell;
      }
