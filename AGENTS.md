...
11. **A new REQUIRED field on a persisted model needs optional typing (`?`) or a backfill migration** — data already on users' disks does not have it, and the typia validator rejects it. TypeScript only protects _new_ data: it errors until you add the field to `DEFAULT_*`, then compiles clean while every existing install still fails validation. Config fields are healed by the `loadAllData` reducer's per-section defaults merge; **entity slices (task, project, tag, note, issueProvider, …) heal nothing** — `auto-fix-typia-errors.ts` blanket-fixes `globalConfig.*` but needs a hand-written branch per entity type (`recreate-fallback.const.ts`). The failure is latent: hydration trusts a snapshot whose schema version matches, so the break only surfaces when an unrelated bump later drags it onto the migration path (#8965 shipped in January, detonated in v18.15.0 as a boot-to-empty-store). Guarded by `src/app/op-log/validation/frozen-state.spec.ts` — if it fails, fix the model, never the fixture. → #9125, #9124.
