...
- **Service size cap:** no service may exceed 1000 LOC. Split by responsibility before crossing the line — extract collaborators, move pure logic to utils or `packages/` — and never grow a service past it. Existing over-limit services (e.g. the sync/conflict-resolution services) are debt to shrink when touched, not a precedent to extend.
